 ...
 *     max_safe_entry[N] = 0
 *     max_safe_entry[i] = min(maxReachableSpeed(max_safe_entry[i+1], mm[i], ...),
 *                             vmax_junction[i])
 *
 ...
 * ─── Merge algorithm ───
 *
 *   Blocks are partitioned into left=[0..L) and right=[L..R):
 ...
 *   Next cycle, the old right blocks become the new left group.
 *
